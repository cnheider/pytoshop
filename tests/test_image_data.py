...
def test_image_valid_size():
    pytoshop.PsdFile(
        ...
        depth=8,
        color_mode=3,
        image_data=image_data.ImageData(channels=np.empty((3, 256, 256), np.uint8)),
    )
...
def test_image_invalid_size():
    psd = pytoshop.PsdFile(
        ...
        depth=8,
        color_mode=3,
        image_data=image_data.ImageData(channels=np.empty((3, 254, 256), np.uint8)),
    )
...
def test_channels_with_other_parameters():
    ...
    for arg in args[0]:
        if arg in ("self", "channels", "compression"):
            continue
        with pytest.raises(ValueError):
            image_data.ImageData(channels=np.empty((3, 0, 0), dtype="u8"), **{arg: 0})
