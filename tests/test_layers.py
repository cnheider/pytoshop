...
def test_futz_with_layer_channels():
    ...
    with open(filename, "rb") as fd:
        ...
        first_layer.set_channel(
            enums.ColorChannel.green, first_layer.get_channel(enums.ColorChannel.red)
        )
        ...
        with pytest.raises(ValueError):
            first_layer.channels = {0: np.empty((200, 100), np.uint8)}
...
def test_channel_image_data_invalid():
    ...
    for arg in args[0]:
        if arg in ("self", "image", "compression"):
            continue
        with pytest.raises(ValueError):
            layers.ChannelImageData(image=np.empty((0, 0), dtype="u8"), **{arg: 0})
