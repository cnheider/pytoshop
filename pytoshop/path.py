...
class PathFillRuleRecord(PathRecord):
    ...
    def read_data(cls, fd, header):
        ...
        if padding != b"\0" * 24:  # pragma: no cover
            raise ValueError("Invalid padding in path fill rule record")
        return cls()
...
class InitialFillRuleRecord(PathRecord):
    def __init__(
        self, all_pixels=False  # type: bool
    ):  # type: (...) -> None
    ...
    def read_data(cls, fd, header):
        ...
        if padding != b"\0" * 22:  # pragma: no cover
            raise ValueError("Invalid padding in initial fill rule record")
...
class ClipboardRecord(PathRecord):
    ...
    def read_data(cls, fd, header):
        # type: (BinaryIO, core.Header) -> PathRecord
        data = fd.read(24)
        top, left, bottom, right, resolution, _ = struct.unpack(str(">iiiiii"), data)
        ...
        return cls(
            top=top, left=left, bottom=bottom, right=right, resolution=resolution
        )
    ...
    def write_data(self, fd, header):
        ...
        util.write_value(fd, "iiiiii", top, left, bottom, right, self.resolution, 0)


class PathResource(object):
    def __init__(self, path_records=[]):  # List[PathRecord]
    ...
    def read(cls, fd, length, header):
        ...
        while fd.tell() + 26 <= end:
            path_records.append(PathRecord.read(fd, header))

        if path_records[0].type != enums.PathRecordType.path_fill_rule_record:
            raise ValueError("Path resource must start with path_fill_rule_record")
