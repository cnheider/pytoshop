...
class TaggedBlock(object):
    ...
    def is_long_length(code, header):
        ...
        return header.version == 2 and code in TaggedBlock._large_layer_info_codes
    ...
    def read(cls, fd, header, padding=1):
        ...
        util.log("code: {}, length: {}, padded_length: {}", code, length, padded_length)

        new_cls = _TaggedBlockMeta.mapping.get(code, GenericTaggedBlock)  # type: ignore
        start = fd.tell()
...
class _SectionDividerSetting(TaggedBlock):
    ...
    def key(self, value):  # type: (Optional[bytes]) -> None
        if value is not None and value not in list(enums.BlendMode):  # type: ignore
...
class MetadataSetting(TaggedBlock):
    def __init__(
        self, datas=None  # type: Optional[Dict[bytes, bytes]]
    ):  # type: (...) -> None
        if datas is None:
            datas = {}
        self.datas = datas
    ...
    def read_data(cls, fd, code, length, header):
        ...
        for i in range(count):
            signature, key, copy, _, entry_length = util.read_value(fd, "4s4sb3sI")
            start = fd.tell()
    ...
    def write_data(self, fd, header):
        ...
        for key, data in self.datas.items():
            util.write_value(fd, "4s4sb3sI", b"8BIM", key, 1, b"\0\0\0", len(data))
            fd.write(data)
