...
class ImageData(object):
    ...
    def __init__(
        ...
        case_b = (
            ...
            or num_channels is not None
            or depth is not None
            or version is not None
        )
        if case_a and case_b:
            raise ValueError("May not provide both channels and other parameters")
        if case_a:
    ...
    def channels(self):  # type: (...) -> np.ndarray
        ...
        try:
            ...
            return image.reshape((self._num_channels, self._height, self._width))
        finally:
            self._fd.seek(tell)
    ...
    def shape(self):  # type: (...) -> Tuple[int, int, int]
        ...
        if self._num_channels is None or self._height is None or self._width is None:
            raise RuntimeError("Internal inconsistency")
    ...
    def write(self, fd, header):
        ...
        if self._fd is None:
            ...
            if self._offset is None or self._size is None:
                raise RuntimeError("Internal inconsistency")
